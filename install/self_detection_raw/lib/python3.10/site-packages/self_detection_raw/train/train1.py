#!/usr/bin/env python3
"""
Training script for Self-Detection Baseline Compensation (V4: Hybrid Residual Architecture)

V4 Architecture:
- Main Stream: MLP (current frame only) for immediate baseline prediction
- Residual Stream: Lightweight GRU (recent N frames) for dynamic error correction
- Output: Y_hat = Y_main + Y_res (with residual constraints)

Loss: Asymmetric Huber Loss to preserve peaks

CLI usage:
    python -m self_detection_raw.train.train1 \
      --data_dir /path/to/logs \
      --glob "robot_data_*.txt" \
      --out_dir train/outputs/run_001 \
      --val_split file --val_ratio 0.2 \
      --use_vel 0 \
      --seq_len 10 \
      --epochs 300 --batch 256 \
      --lr 1e-3 --wd 1e-4 \
      --main_hidden 128 --res_hidden 64 \
      --dropout 0.1 --res_scale 0.1 \
      --seed 42
"""
...
class SequenceDataset(Dataset):
    """PyTorch Dataset for sequence-based training (with buffer for residual stream)."""
    ...
    def __init__(
        ...
        """
        Args:
            X: (N, D) input features [sin(j1..j6), cos(j1..j6)] or with velocity
            Y: (N, C) target raw values (C channels)
            seq_len: Sequence length for residual stream
    ...
    def __getitem__(self, idx):
        # Current frame
        x_current = self.X_norm[idx + self.seq_len - 1]
        
        # Sequence for residual stream (recent N frames)
        x_seq = self.X_norm[idx:idx + self.seq_len]
        
        # Target
        y_target = self.Y_norm[idx + self.seq_len - 1]
        
        return (
            torch.from_numpy(x_current),
...
def asymmetric_huber_loss(pred, target, s, beta=1.0, w_over=3.0):
    """
    Asymmetric Huber Loss for peak preservation.
    
    Args:
        pred: (batch, C) predicted values
        target: (batch, C) target values
        s: (C,) peak direction signs (+1 or -1 for each channel)
        beta: Huber loss threshold (default: 1.0)
        w_over: Weight multiplier for over-prediction in peak direction (default: 3.0)
    
    Returns:
        Scalar loss value
    """
    err = pred - target  # (batch, C)
    ...
    # s*err > 0: over-prediction in peak direction (risk of cutting peaks)
    s_expanded = s.view(1, -1)  # (1, C)
    over = (err * s_expanded > 0).float()  # (batch, C)
    w = 1.0 + (w_over - 1.0) * over  # (batch, C)
...
def evaluate(model, dataloader, dataset, device):
    ...
    for x_current, x_seq, y_target in dataloader:
        x_current = x_current.to(device)
        x_seq = x_seq.to(device)
        pred_norm = model(x_current, x_seq)
...
def train_epoch(model, dataloader, criterion, s, device):
    """Train for one epoch."""
    model.train()
    total_loss = 0.0
    n_batches = 0
    
    for x_current, x_seq, y_target in tqdm(dataloader, desc='Training', leave=False):
        x_current = x_current.to(device)
        x_seq = x_seq.to(device)
        y_target = y_target.to(device)
        
        # Forward
        pred = model(x_current, x_seq)
        loss = criterion(pred, y_target, s)
        ...
        # Update (assuming optimizer is in outer scope)
        # Note: optimizer.step() and zero_grad() should be called outside
...
def main():
    parser = argparse.ArgumentParser(description='Train V4: Hybrid Residual Architecture')
    ...
    parser.add_argument('--use_vel', type=int, default=0,
                        help='Use joint velocities (0=sin/cos only, 1=with velocity)')
    
    # Sequence
    parser.add_argument('--seq_len', type=int, default=10,
                        help='Sequence length for residual stream (default: 10)')
    ...
    # Model
    parser.add_argument('--main_hidden', type=int, default=128,
                        help='Main stream hidden dimension')
    parser.add_argument('--res_hidden', type=int, default=64,
                        help='Residual stream GRU hidden dimension (lightweight)')
    parser.add_argument('--dropout', type=float, default=0.1,
                        help='Dropout rate')
    parser.add_argument('--res_scale', type=float, default=0.1,
                        help='Residual scaling factor (keep residual small, default: 0.1)')
    parser.add_argument('--res_clip', type=float, default=None,
                        help='Residual clipping value (None = no clipping)')
    
    # Loss
    parser.add_argument('--huber_beta', type=float, default=1.0,
                        help='Huber loss threshold (default: 1.0)')
    parser.add_argument('--w_over', type=float, default=3.0,
                        help='Weight multiplier for over-prediction in peak direction (default: 3.0)')
    parser.add_argument('--peak_direction', type=str, default='+1,+1,+1,+1,+1,+1,+1,+1',
                        help='Peak direction signs for each channel (comma-separated, +1 or -1, default: all +1)')
    ...
    # Parse peak direction
    peak_dirs = [int(x.strip()) for x in args.peak_direction.split(',')]
    if len(peak_dirs) != 8:
        raise ValueError(f"peak_direction must have 8 values, got {len(peak_dirs)}")
    s = torch.tensor(peak_dirs, dtype=torch.float32, device=device)
    ...
    # Store in args for model loading
    args.in_dim = in_dim
    ...
    print(f"  Main stream: {sum(p.numel() for p in model.main_stream.parameters()):,}")
    print(f"  Residual stream: {sum(p.numel() for p in model.res_gru.parameters()) + sum(p.numel() for p in model.res_proj.parameters()):,}")
    
    # Loss function (asymmetric Huber)
    def criterion(pred, target, s):
        return asymmetric_huber_loss(pred, target, s, beta=args.huber_beta, w_over=args.w_over)
    ...
    print("\n" + "="*60)
    print("Starting training (V4: Hybrid Residual Architecture)...")
    print("="*60)
    print(f"Main stream: MLP ({args.main_hidden} hidden)")
    print(f"Residual stream: GRU ({args.res_hidden} hidden, seq_len={args.seq_len})")
    print(f"Residual scale: {args.res_scale}")
    print(f"Peak directions: {peak_dirs}")
    print("="*60)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for x_current, x_seq, y_target in tqdm(train_loader, desc=f'Epoch {epoch}', leave=False):
            x_current = x_current.to(device)
            x_seq = x_seq.to(device)
            y_target = y_target.to(device)
