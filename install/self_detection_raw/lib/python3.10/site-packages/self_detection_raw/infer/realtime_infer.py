...
class RealtimeInferNode(Node):
    """Real-time self-detection compensation node."""

    def __init__(self):
        super().__init__('realtime_infer_raw')
        ...
        if not model_file:
            ...
            if not model_file:
                ...
                self.get_logger().error("Please train a model first or specify model_file parameter.")
        ...
        for i in range(N_SENSORS):
            self.comp_pubs.append(
                self.create_publisher(
                    Range,
                    f'/compensated_raw_distance{i+1}',
                    10
                )
            )
        ...
        filename = f'compensated_raw_{model_name}_{now}.txt'
        ...
        self.get_logger().info("Real-time Self Detection Compensation (ModelB)")
    ...
    def timer_callback(self):
        ...
        X = np.concatenate([sin_j, cos_j], axis=0).astype(np.float32)
...
def find_available_models():
    ...
    train_outputs_dir = os.path.join(base_dir, 'self_detection_raw', 'train', 'outputs')
    package_outputs_dir = os.path.join(base_dir, 'outputs')
    
    model_files = []
    
    # First, check train/outputs/ (where train.py saves models)
    ...
    # Then, check outputs/ (package root, for backward compatibility)
    if os.path.exists(package_outputs_dir):
        for run_dir in sorted(glob.glob(os.path.join(package_outputs_dir, 'run_*')), reverse=True):
            model_path = os.path.join(run_dir, 'model.pt')
            if os.path.exists(model_path):
                model_files.append(model_path)
...
def main(args=None):
    ...
    if not model_file_from_param or not model_file_from_param.strip():
        ...
        if model_files:
            ...
            print("\n" + "=" * 60)
            print(f"[INFO] Auto-selected latest model: {model_name}")
            ...
            print("[ERROR] No model files found. Please train a model first or specify model_file parameter.")
            sys.exit(1)
    else:
